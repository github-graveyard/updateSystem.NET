...
namespace updateSystemDotNet.Internal.updateUI.Controls {
	...
	internal class statusLabel : Control {
			...
			if (_state != statusLabelStates.Waiting) {
				...
				switch (_state) {
					case statusLabelStates.Success:
						image = resourceHelper.getResourceImage("statuslabel_success.png");
						break;
					case statusLabelStates.Failure:
						image = resourceHelper.getResourceImage("statuslabel_error.png");
						break;
		...
		private Image getCircleImage() {
			...
			using (Graphics g = Graphics.FromImage(circle)) {
				g.DrawImage(
					resourceHelper.getResourceImage("statuslabel_progress.png"),
