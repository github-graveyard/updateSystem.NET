...
namespace updateSystemDotNet.Administration.Core.DragDrop {
	...
	public static class DragSourceHelper {
		...
		public static IDataObject CreateDataObject() {
			return new System.Windows.Forms.DataObject(new DataObject());
		}
		...
		public static void UnregisterDefaultDragSource(Control control) {
			if (s_dataContext.ContainsKey(control)) {
				...
				var dataObjectCOM = (System.Runtime.InteropServices.ComTypes.IDataObject) entry.data;
		...
		private class AdviseSink : IAdviseSink {
			...
			public void OnDataChange(ref FORMATETC format, ref STGMEDIUM stgmedium) {
				...
				object odd = ((System.Runtime.InteropServices.ComTypes.IDataObject) data).GetDropDescription();
				if (odd != null)
		...
		#region Nested type: DragSourceEntry
		...
		#region Nested type: DropDescriptionFlags
